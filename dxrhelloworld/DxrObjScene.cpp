...
void ObjScene::LoadModel() {
  ...
  printf("[tinyobj] %zu shapes, %zu materials, ret=%d, loading took %.2fs\n", shapes.size(), materials.size(), ret, millis / 1000.0f);
  ...
  for (int i = 0; i < materials.size(); i++) {
    ...
    if (std::filesystem::exists(dt)) {
      ...
      unsigned char* image = stbi_load(dt.c_str(), &tex_w, &tex_h, &tex_channels, STBI_rgb);
      assert(image != 0);
      int pixel_idx = tex_w * (tex_h >> 1) + tex_w >> 1;
      float r = image[pixel_idx * tex_channels + 0] / 255.0f;
      float g = image[pixel_idx * tex_channels + 1] / 255.0f;
      float b = image[pixel_idx * tex_channels + 2] / 255.0f;
...
void ObjScene::CreateRaytracingPipeline() {
...
void ObjScene::CreateRaytracingOutputBufferAndSRVs() {
  // Output buffer
...
void ObjScene::CreateShaderBindingTable() {
  ...
  UINT8* ptr = nullptr;
  rt_sbt_storage->Map(0, nullptr, (void**)(&ptr));
