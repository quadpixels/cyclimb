...
void ObjScene1::InitDX12Stuff() {
  ...
  // SRV/UAV Heap
  {
    ...
    heap_desc.NumDescriptors = 2;  // [0]=output [1]=BVH
...
void ObjScene1::CreateRTPipeline() {
  ...
  // 2. RTPSO
  {
    ...
    local_rootsig_assoc.pExports = &(dxil_lib_exports[0].Name);  // L"RayGen"
...
void ObjScene1::CreateShaderBindingTable() {
  ...
  PrintShaderIdentifier("Raygen  ", raygen_shader_id);
  PrintShaderIdentifier("Hitgroup", hitgroup_id);
  PrintShaderIdentifier("Miss    ", miss_shader_id);
...
void ObjScene1::Render() {
  ...
  // 1) If only Descriptor table is not set, DXGI_ERROR_DEVICE_HUNG will occur
  // 2) If only RS is not set, EXECUTION ERROR #708: SET_DESCRIPTOR_TABLE_INVALID will occur
  // 3) If both are not set, the entire window becmoes all-black
  command_list->SetComputeRootSignature(global_rootsig);
  command_list->SetComputeRootDescriptorTable(0, handle_uav);

  D3D12_DISPATCH_RAYS_DESC desc{};

  // If RayGenerationShaderRecord is unassigned, EXECUTION ERROR #1161: DISPATCH_RAYS_INVALID will occur
  ...
  // If the MissShaderTable is unassigned, DXGI_ERROR_DEVICE_HUNG will occur
  desc.MissShaderTable.StartAddress = miss_sbt_storage->GetGPUVirtualAddress();
  desc.MissShaderTable.SizeInBytes = 64;

  // If the HitGroupTable is unassigned, the triangle becomes all-black
