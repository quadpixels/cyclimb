...
namespace omm
{
  ...
  void GpuBakeNvrhiImpl::InitStaticBuffers(nvrhi::CommandListHandle commandList)
  {
	...
	{ // NVRHI has trouble binding zero RTV's
	  nvrhi::TextureHandle virtualTexture;
	  {
		...
		// Vulkan backend must not transition render target textures to Common,
		// because Common maps to VK_IMAGE_LAYOUT_UNDEFINED for images.
		// Keep this dummy framebuffer attachment in RenderTarget state.
	...
	const omm::Gpu::DispatchChain* dispatchDesc)
  {
	...
	if (rtv)
	  commandList->beginTrackingTextureState(rtv, nvrhi::AllSubresources, nvrhi::ResourceStates::RenderTarget);
